...
def test_get_trending_keywords(mock_trend_req):
    ...
    mock_instance.trending_searches.return_value = pd.DataFrame(
        {"trending_searches": ["keyword1", "keyword2"]}
    )
...
def test_find_uncovered_topics(
    mock_get_search_volume,
    mock_is_topic_uncovered,
    mock_get_autocomplete_suggestions,
    mock_get_trending_keywords,
):
    mock_get_trending_keywords.return_value = ["topic1", "topic2"]
    mock_get_autocomplete_suggestions.side_effect = (
        lambda x, y: mock_autocomplete_suggestions.get(x, [])
    )
    mock_is_topic_uncovered.side_effect = (
        lambda x, threshold=0: x != "topic2 suggestion1"
    )  # Assume topic2 suggestion1 is covered
    ...
    expected = [
        ...
        # 'topic2 suggestion1' is covered, so it's excluded
        {
            "keyword": "topic3",
            "search_volume": 200,
        },  # Assuming no suggestions for topic3
    ]
